...
private extension CaptureViewModel {
  ...
  func updateGeometry(_ size: CGSize) {
    container.pipeline.updateGeometry(
      viewSize: size,
      orientation: displayOrientation(for: size),
      fillMode: .aspectFit
    )
  }

  func displayOrientation(for size: CGSize) -> DisplayOrientation {
    guard let windowScene = UIApplication.shared.connectedScenes
      .compactMap({ $0 as? UIWindowScene })
      .first(where: { $0.activationState == .foregroundActive })
    else {
      return fallbackOrientation(for: size)
    }
  ...
  func fallbackOrientation(for size: CGSize) -> DisplayOrientation {
    size.width > size.height ? .landscapeRight : .portrait
  }
