...
private struct CapturePreviewArea: View {
  ...
  var body: some View {
    GeometryReader { proxy in
      ...
      .clipped()
      .onAppear {
        send(.geometryChanged(proxy.size))
      }
...
private struct MetricsPanel: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      MetricRow(
        name: "sharpness",
        value: state.sharpnessText,
        state: state.sharpnessState
      )
      ...
      Button(action: fire) {
        Text(state.shutterText)
          ...
          .overlay {
            Circle()
              .strokeBorder(
                state.isShutterEnabled
                ? Color.primary
                : Color.secondary.opacity(0.35),
                style: StrokeStyle(
                  lineWidth: state.isShutterEnabled ? 2 : 1,
                  dash: state.isShutterEnabled
                  ? []
                  : [3, 3]
                )
              )
