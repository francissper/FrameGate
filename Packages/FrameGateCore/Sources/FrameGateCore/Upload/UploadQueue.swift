...
public actor UploadQueue {
  ...
  public func drainOnce() async {
    ...
    let manifestPath = storage.appendingPathComponent(record.manifestFilename).path
    let framePath = storage.appendingPathComponent(record.frameFilename).path

    guard let manifest = FileManager.default.contents(atPath: manifestPath),
          let frame = FileManager.default.contents(atPath: framePath) else {
      return
    }
