...
extension BackoffPolicyTests {
    ...
    func testJitterIsAppliedAfterTheCapSoCappedRetriesStillSpread() {
        let low = policy(jitter: 0.5)
        let high = policy(jitter: 1.5)

        XCTAssertEqual(low.delay(afterAttempt: 12, retryAfter: nil), 30)
        XCTAssertEqual(high.delay(afterAttempt: 12, retryAfter: nil), 90)

        // Applying jitter before the cap would pin every capped retry to exactly
        // 60s, losing the spread precisely when the most records are waiting.
    }
