...
extension CapturePipelineTests {
    ...
    func testFiringWhileArmedReturnsTheCurrentStep() throws {
        let pipeline = CapturePipeline(source: try makeSource(), plan: try makePlan(holdFrames: 2))
        pipeline.updateGeometry(viewSize: CGSize(width: 390, height: 844),
                                orientation: .portrait, fillMode: .aspectFit)
    ...
    func testFiringWhileNotArmedReturnsNil() throws {
        let pipeline = CapturePipeline(source: try makeSource(), plan: try makePlan())
        XCTAssertNil(pipeline.fire(), "nothing has run yet, the gate cannot be armed")
    }
}
